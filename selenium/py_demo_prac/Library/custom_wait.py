...
def wait_(func=None,*,visibility=True,enabled=True,timeout=60,is_alert=False):
    if func is None:
        return partial(wait_,visibility=visibility,enabled=enabled,timeout=timeout,is_alert=is_alert)
    ...
    def wrapper(*args, **kwargs):
        ...
        if visibility:
            ...
            if enabled:
                wait.until(element_to_be_enabled(locator_type, locator),
                           message=f'{locator} is enabled after timeout period of {timeout} seconds')
                if func.__name__ == 'click_element':
                    wait.until(ec.element_to_be_clickable((locator_type, locator)),
                               message=f'{locator} is not clickable after timeout period of {timeout} seconds')
            return func(*args, **kwargs)
        else:
            time.sleep(3)
